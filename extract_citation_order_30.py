...
def find_citations(text):
    ...
    biblio_start = text.lower().find('bibliography')
    if biblio_start == -1:
        biblio_start = len(text)
    
    # Find all numeric citations [X]
    citation_pattern = r'\[(\d+)\]'
    citations = re.findall(citation_pattern, text)
    
    # Get unique citations in order of appearance (before bibliography)
    seen = set()
    ordered_citations = []
